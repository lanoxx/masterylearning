/** source: https://github.com/arendjr/text-clipper */
 ...
 */
function text_clipper_clipHtmlclip(string, maxLength) {
    ...
    return options.html ? text_clipper_clipHtml(string, maxLength, options) : text_clipper_clipPlainText(string, maxLength, options);
}

function text_clipper_clipHtml(string, maxLength, options) {
                ...
                while (true) {
                    ...
                    if (isAttributeValue) {
                        if (attributeQuoteCharCode) {
                            ...
                            if (text_clipper_isWhiteSpace(_charCode)) {
                                isAttributeValue = false;
                        ...
                        while (text_clipper_isWhiteSpace(string.charCodeAt(endIndex + 1))) {
                        ...
                        var tagNameEndIndex = Math.min(text_clipper_indexOfWhiteSpace(string, tagNameStartIndex), endIndex);
    ...
    if (numChars > maxLength) {
        var nextChar = text_clipper_takeCharAt(string, i);
        ...
        if (peekIndex && (peekIndex === string.length || text_clipper_isLineBreak(string, peekIndex))) {
        ...
        if (i < string.length) {
            if (!options.breakWords) {
                ...
                for (var j = i - 1; j >= 0; j--) {
                    ...
                    if (_charCode2 === TAG_CLOSE_CHAR_CODE || _charCode2 === SEMICOLON_CHAR_CODE) {
                        ...
                        i = j;
                        break;
                    } else if (text_clipper_isWhiteSpace(_charCode2)) {
                        i = j + 1;
                        break;
                    }
                }
            ...
            var result = string.slice(0, i) + (text_clipper_isLineBreak(string, i) ? '' : options.indicator);
...
function text_clipper_clipPlainText(string, maxLength, options) {
    var maxLines = options.maxLines;
            ...
            if (!options.breakWords) {
                ...
                for (var j = i - 1; j >= 0; j--) {
                    ...
                    if (_charCode3 === NEWLINE_CHAR_CODE) {
                        ...
                        nextChar = '\n';
                        break;
                    } else if (text_clipper_isWhiteSpace(_charCode3)) {
                        i = j + 1;
                        break;
                    }
                }
...
function text_clipper_indexOfWhiteSpace(string, fromIndex) {
    var length = string.length;

    for (var i = fromIndex; i < length; i++) {
        if (text_clipper_isWhiteSpace(string.charCodeAt(i))) {
            return i;
        }
    }
...
function text_clipper_isLineBreak(string, index) {
...
function text_clipper_isWhiteSpace(charCode) {

    return charCode === 9 || charCode === 10 || charCode === 12 || charCode === 13 || charCode === 32;
}

function text_clipper_takeCharAt(string, index) {
